...
public class MainComponent extends CustomComponent {
	...
	private enum Location {
		...
		public static Location getByName(String location) {
	...
	private static final String LOC_COOKIE = "LOC";
	private static final String LAB_COOKIE = "LAB";
	 ...
	 * @param request
	 */
	public MainComponent(HttpServletRequest request) {
		buildMainLayout();
		...
		initializeCalendar();
		initializeComboBoxs(request);
		initializeButtonsAndInputs();
	}

	protected void saveCookie() {
		String locVal = ((Location) locationBox.getValue()).getLocation();
		saveCookie(LOC_COOKIE, locVal);
		String labVal = (String) labBox.getValue();
		saveCookie(LAB_COOKIE, labVal);
	}

	private void saveCookie(final String cookieKey, final String cookieVal) {
	...
	private void initializeButtonsAndInputs() {
		...
		saveButton.addListener(new ClickListener() {
			private static final long serialVersionUID = -8230469488810055947L;

			@Override
			public void buttonClick(ClickEvent event) {
				saveCookie();
				getWindow().showNotification("Location and Lab selections saved.");
			}
		});
	}
	...
	private void showEventPopup(CalendarEvent event, boolean newEvent) {
		...
		if (labBox.getValue() == null || ((String) labBox.getValue()).length() == 0) {
			getWindow().showNotification("Please select lab.", "", Notification.TYPE_ERROR_MESSAGE);
			return;
		}
	...
	private void initializeComboBoxs(HttpServletRequest request) {
		...
		for (int i = 0; i < cookies.length; i++) {
			if (cookies[i].getName().equals(LOC_COOKIE)) {
				...
				saveCookie(LOC_COOKIE, locCookie.getLocation());
			} else if (cookies[i].getName().equals(LAB_COOKIE)) {
				labCookie = cookies[i].getValue();
				saveCookie(LAB_COOKIE, labCookie);
			}
		}
		initializeLocaleBox(locCookie);
		initializeTimeZoneBox(locCookie);
		initializeLocationBox(locCookie);
		initializeCalendarIdBox(labCookie);
	}

	private void initializeCalendarIdBox(String labCookie) {
		...
		labBox.addListener(new ValueChangeListener() {
			...
			public void valueChange(ValueChangeEvent event) {
				updateLab((String) event.getProperty().getValue());
			}
		});
	}
	...
	private void initializeLocationBox(Location locCookie) {
		...
		Location defaultLocation = DEFAULT_LOC;
		if (locCookie != null) {
			defaultLocation = locCookie;
		}
		locationBox.select(defaultLocation);
	...
	private void initializeTimeZoneBox(Location locCookie) {
		...
		String defaultTimezone = DEFAULT_LOC.getTimezone();
		if (locCookie != null) {
			defaultTimezone = locCookie.getTimezone();
		}
		timezoneBox.select(defaultTimezone);
	...
	private void initializeLocaleBox(Location locCookie) {
		...
		if (locCookie != null) {
			defaultLocale = locCookie.getLocale();
		}
		localeBox.select(defaultLocale);
